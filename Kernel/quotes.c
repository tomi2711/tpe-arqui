...
int kinitQuotes(){
    ...
    return SCREENSAVER_CHANGE_TIME;
}
...
void showRandomScreensaver(){
    ...
    int cursorJ = 9;
    int cursorI = 0;
    int screenWidth = kgetScreenWidth();
    int length = kstrlen(phrase);

    if((screenWidth-length) < 10){

        int i;
        for(i=30; phrase[length-i] != ' '; i++);

        int subLength = length - i;
        cursorI = (screenWidth-subLength)/2;
        ...
        cursorJ++;
        int cursorI = (screenWidth - i)/2;
        kmoveCursor(cursorI,cursorJ);
        while(subLength<length){
            kputChar(phrase[subLength++]);
        }
        kmoveCursor(cursorI,cursorJ+1);
        kputNewLine();
    } else {
